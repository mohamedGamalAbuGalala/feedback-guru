...
export async function POST(request: NextRequest) {
  try {
    ...
    const { workspaceId, webhookUrl, channel } = slackSchema.parse(body);
    ...
    try {
      const testMessage = {
        text: "🎉 Slack integration successfully connected to Feedback Guru!",
        channel,
      };
      ...
      if (!testResponse.ok) {
        return NextResponse.json(
          { error: "Failed to send test message to Slack. Please check your webhook URL." },
          { status: 400 }
        );
      }
    } catch (error) {
